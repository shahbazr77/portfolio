  ...
  "use strict";
	...
	var Rewall = {
		...
		init: function(){
			...
			/* Load More Portfolio Projects */
			Rewall.loadPortfolioPosts();
		...
		loadPortfolioPosts: function(){
			$('.portfolio_list .load_more a').on('mousedown',function(){
				var element 	= $(this);
				var text 		= element.find('.text');
				if(element.hasClass('done')){
					element.addClass('hold');
			...
			$('.portfolio_list .load_more a').on('click',function(){
				var element 	= $(this);
				var text 		= element.find('.text');
				var portfolio 	= element.closest('.portfolio_list');
				var masonry 	= portfolio.find('.fn__masonry');

				// If everything is already loaded, mark DONE immediately.
				var remainingHiddenInit = portfolio.find('.be_animated:not(.done)').length;
				if(remainingHiddenInit === 0){
					element.addClass('done');
					text.text(element.attr('data-done'));
					return false;
				}
				...
				setTimeout(function(){
					// Only reveal cards that are not already visible
					var listItem = portfolio.find('.be_animated:not(.done)').slice(0,4);
					listItem.each(function(i, e){
						setTimeout(function(){
							$(e).addClass('fadeInTop done');
						}, (i*100));	
					...
					// Reflow isotope layout after showing hidden portfolio cards.
					// (Must happen after the last item becomes visible; otherwise isotope stacks them.)
					var lastIndex = listItem.length - 1;
					var relayoutDelay = (Math.max(lastIndex, 0) * 100) + 150;
					setTimeout(function(){
						if($().isotope){
							masonry.isotope('layout');
						}

						// Update button label after the full batch becomes visible.
						var remainingHidden = portfolio.find('.be_animated:not(.done)').length;
